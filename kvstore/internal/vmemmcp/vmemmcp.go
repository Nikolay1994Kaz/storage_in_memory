...
type Config struct {
	...
	StartHint string
	// Source — происхождение фактов, записанных через этот адаптер. Ставится
	// НА СТОРОНЕ АДАПТЕРА и сознательно НЕ выведен в схему инструмента:
	// провенанс, который объявляет сам пишущий агент, стоит ровно столько,
	// сколько стоит доверие к агенту, а весь смысл слоя — уцелеть в сценарии,
	// где агент уже действует по подсунутой инструкции. Атакующий, влияющий
	// на агента, не должен уметь подписать свой факт чужим источником.
	// Пустая строка → SOURCE не передаётся, сервер штампует "unknown".
	// Открытая дверь: заявленный агентом под-источник («это из письма») —
	// полезная информация, но её место рядом с доверенным каналом, а не
	// вместо него; отдельным полем, когда понадобится.
	Source string
}
...
func (s *server) remember(raw json.RawMessage) callResult {
	...
	// Источник — из конфигурации адаптера, не из аргументов вызова (см. Config.Source).
	if s.cfg.Source != "" {
		args = append(args, bulk("SOURCE"), bulk(s.cfg.Source))
	}
	v, err := s.do(args)
	if err != nil {
