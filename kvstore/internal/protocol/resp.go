...
// Buffered возвращает количество байт, готовых для чтения в буфере.
//
// Используется для pipeline: после чтения первой команды
// проверяем, есть ли в буфере ещё данные.
// Если Buffered() > 0 — клиент прислал несколько команд в одном пакете.
//
// bufio.Reader читает из TCP сокета БОЛЬШИМИ порциями (4KB по умолчанию).
// Если клиент прислал 16 команд за раз (pipeline=16),
// все 16 окажутся в буфере после первого системного вызова read().
