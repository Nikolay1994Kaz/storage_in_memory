...
//
// ─────────────────────────────────────────────────────────────────────────
// РЕЗУЛЬТАТ МУТАЦИОННОЙ ПРОВЕРКИ 31.07: 15 поймано, 4 ЭКВИВАЛЕНТНЫ.
// Записано, чтобы следующий прогон не разбирал это заново и не принял
// эквивалентность за дыру в тестах.
//
//	«отсутствие атрибута считать скоупом» (снять проверку c == attrMissing в
//	    scopeColumnReader) — ЭКВИВАЛЕНТНА: attrMissing = ^uint32(0)
//	    (attr.go:25), и соседняя проверка int(c) >= len(vals) его покрывает.
//	    Явная проверка оставлена намеренно — она не должна зависеть от того,
//	    что сентинел случайно вне диапазона словаря.
//
