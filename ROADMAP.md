...
Высокий приоритет (для production-ready MVP)
#	Задача	Сложность	Зачем
1	VSIM.DEL — удаление вектора	Средняя	Без удаления нельзя обновлять данные
2	Snapshot для векторов — включить в compaction	Лёгкая	Сейчас WAL растёт бесконечно
3	Тесты WAL recovery — автоматизировать	Лёгкая	Убедиться что сериализация битово точная
Средний приоритет (качество/скорость)
#	Задача	Сложность	Зачем
4	Epoch visited вместо map	Средняя	−15% CPU на search path
5	Нормализация при вставке	Лёгкая	CosineDistance через Euclidean
6	Batch VSIM.ADD — пакетная вставка	Лёгкая	Быстрая загрузка данных
Низкий приоритет (масштабирование)
#	Задача	Сложность	Зачем
7	Arena storage для графа (Phase 3b)	Высокая	GC не сканирует данные
8	SIMD EuclideanDistance	Высокая	+2-3x скорость вычислений
9	Шардирование векторов по кластеру	Высокая	Горизонтальное масштабирование
