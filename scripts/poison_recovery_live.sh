...
# qfield <квитанция> <имя поля> — ответ-квитанция приходит плоским списком
# «имя, значение, …», и поле достаётся ПО ИМЕНИ, а не по позиции: порядок пар
# — деталь протокола, а разбор по номеру строки молча поехал бы при первом же
# добавленном поле.
...
RECEIPT=$(r VMEM.QUARANTINE "$SCOPE" SOURCE "$SUSPECT")
REVOKED=$(qfield "$RECEIPT" revoked)
STILL=$(qfield "$RECEIPT" still_trusted)
echo "    VMEM.QUARANTINE $SCOPE SOURCE $SUSPECT → отозвано: $REVOKED"
# ⭐Вторая половина ответа, без которой первая не значит «вылечено»: сколько
# фактов этого источника память ВСЁ ЕЩЁ считает истиной. Ноль здесь — это
# «в пределах предиката больше нечего снимать», и он проверяется ниже, а не
# подразумевается.
echo "    остаток по источнику (движок называет сам): $STILL"
[ "$STILL" = "0" ] || { echo "    ⚠движок сообщает о непролеченном остатке — демо не вправе идти дальше"; exit 1; }
echo
