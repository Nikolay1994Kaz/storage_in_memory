...
Notes per the unfreeze checklist: gates — `VMEM.REMEMBER` and
`VMEM.QUARANTINE` are memory-growing + write (revocation appends a new version
rather than deleting, so it genuinely grows), `VMEM.FORGET` is write-only
