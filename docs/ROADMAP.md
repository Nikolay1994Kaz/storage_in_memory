...
Что сделано (Выполнено в рамках проекта Molten):
#	Задача	Сложность	Статус / Зачем
1	VSIM.DEL — удаление вектора	Средняя	✅ Выполнено (Без удаления нельзя обновлять данные)
2	Snapshot для векторов — включить в compaction	Лёгкая	✅ Выполнено (Сейчас WAL растёт бесконечно)
3	Тесты WAL recovery — автоматизировать	Лёгкая	✅ Выполнено (Убедиться что сериализация битово точная)
4	Epoch visited вместо map	Средняя	✅ Выполнено (−15% CPU на search path)
5	Нормализация при вставке	Лёгкая	✅ Выполнено (CosineDistance через Euclidean)
6	Batch VSIM.ADD — пакетная вставка	Лёгкая	✅ Выполнено (Быстрая загрузка данных)
7	Arena storage для графа (Phase 3b)	Высокая	✅ Выполнено (TCMalloc для связей графа, GC не сканирует данные)
8	SIMD EuclideanDistance	Высокая	✅ Выполнено (AVX2 ассемблерная реализация, +2-3x скорость)
9	Шардирование векторов по кластеру	Высокая	✅ Выполнено (Горизонтальное масштабирование в кластере)
